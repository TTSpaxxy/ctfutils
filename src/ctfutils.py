...
error = {1:"Ambiguous command", 2:"Invalid command", 3:"Not implemented", 4:"Unrecognized argument", 5:"Missing required argument"}
...
Interfaces with the utils package to actually run the command
    ...
    if len(matches) == 0: return 2 #Command was not found
    if len(matches) > 1: return 1 #Input could match different commands
...
if __name__ == "__main__":
    ...
    if argc <= 2:
        sys.exit("Usage: %s [Command] <Arguments>" % argv[0])
    #End if
